...
useEffect(() => {
  const handleKeyPress = (event) => {
    ...
    const pressedKey = synthKeys.find(
      (synthKey) => synthKey.computerKey === key
    );
    if (pressedKey) {
      setSynthKeys((prevKeys) =>
        prevKeys.map((key) =>
  ...
  const handleKeyUp = (event) => {
    const key = event.key.toLowerCase();
    const releasedKey = synthKeys.find(
      (synthKey) => synthKey.computerKey === key
    );
    if (releasedKey) {
      setSynthKeys((prevKeys) =>
        prevKeys.map((key) =>
          key === releasedKey ? { ...key, keyPressed: false } : key
        )
      );
    }
  };
  ...
  return (
    <div className='wrapper'>
      <div className='main-container'>
        ...
        <Keys showKeys={showKeys} onMouseDown={handleMouseDown}
          onMouseUp={handleMouseUp} synthKeys={synthKeys} />
      </div>
    </div>
  )
}
